 ...
 *             last array elements */
   ...
   sizeLocal = (long long) sqrt((double) n);
 ...
 *       not all held by process 0 */
   ...
   markedLocal = (char *) malloc (sizeLocal);

   if (marked == NULL || markedLocal) {
   ...
   for (i = 0; i < sizeLocal; i++) markedLocal[i] = 0;
   ...
   do {
        if (prime * prime > low_value) {
            ...
            if ((low_value + first) % 2 == 0  ) { //here is where we skip evens
                first += prime;
            }
            first /= 2 ;
            }

        firstLocal = (prime * prime - 3) / 2; 

        for (i = first; i < size; i += prime) marked[i] = 1;
        for (i = firstLocal; i < sizeLocal; i += prime) markedLocal[i] = 1;
        while (markedLocal[++index]);
        prime = 2 * index + 3;
   ...
   if (!id) {
       ++global_count; // we skipped 2 earlier
